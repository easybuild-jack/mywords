...
compile v. 编译；编纂`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="glass-panel w-full max-w-4xl rounded-3xl border border-white/10 p-6 space-y-6 shadow-2xl text-white">
        {/* 顶部标题与关闭 */}
        ...
        <div className="flex gap-2 p-1 bg-white/[0.04] rounded-xl border border-white/10">
          <button
            ...
            className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all flex items-center justify-center gap-2 ${
              activeTab === 'text' ? 'bg-primary text-[#0B0C0E]' : 'text-[#9CA3AF] hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all flex items-center justify-center gap-2 ${
              activeTab === 'file' ? 'bg-primary text-[#0B0C0E]' : 'text-[#9CA3AF] hover:text-white'
            }`}
          >
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            {targetMode === 'existing' ? (
              ...
              <>
                ...
                内置词库无法作为目标，它的内容是从词库文件按需加载的。
              </>
            ) : (
              <>
                <span className="text-gray-300">「并入已有词库」暂不可选：你还没有自定义词库。</span>
                本次先新建一本，之后再导入就能选择并入了。内置词库无法作为目标，它的内容是从词库文件按需加载的。
              </>
            )}
          </p>
        </div>
