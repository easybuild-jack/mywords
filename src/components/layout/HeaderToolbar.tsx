      ...
      <div className="glass-card rounded-2xl px-5 py-2.5 flex items-center gap-4 text-sm max-w-5xl shadow-[0_8px_30px_rgba(0,0,0,0.4)] border border-white/10">
        ...
        {isErrorPracticeActive ? (
          <Tooltip content="错词攻坚强制连续 3 次无误默写" side="bottom">
            <div className="bg-destructive/10 border border-destructive/30 text-destructive rounded-md h-8 px-3 inline-flex items-center text-xs font-mono font-bold cursor-default">
              循环 3次 (3-Streak)
            </div>
          </Tooltip>
        ) : (
        ...
        <Tooltip content="Restart (回到第一个单词)" side="bottom">
          <button
            onClick={restartUnit}
            className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-primary text-[#0B0C0E] font-bold text-xs btn-neon-glow hover:bg-primary-hover transition-all cursor-pointer whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-primary/60 focus:ring-offset-2 focus:ring-offset-background"
          >
